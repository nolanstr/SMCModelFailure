...
SSqk = np.sum((model.evaluate_equation_at(x).flatten() - y_noisy.flatten())**2)
var = SSqk / n

priors[-1].alpha = 0.5 * (ns + n)
priors[-1].beta = np.ones(particles) * 0.5 * (ns*var + SSqk)
